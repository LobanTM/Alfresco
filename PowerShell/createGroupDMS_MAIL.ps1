...
foreach ($OU in Find-ADMemberOUsForOU "*" $RootDomainOU){
...
function Find-ADDMSGroupForOU($name, $RootOU){             
     if (! (Get-ADGroup -filter {name -like $name} -SearchBase $RootOU -Properties * -SearchScope OneLevel)){
                #New-ADGroup $name -path $UOSOU -GroupScope Global
                }
