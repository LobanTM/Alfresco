...
#путь и имя файла 1С
$CSVpath="D:\IT\admins\olegyu\"
$UserCSVFile="ad_user.csv"
#$DepCSVFile="ad_dep.csv"

$LogsFile = "D:\IT\admins\olegyu\result\logs.txt"
...
foreach ($UOSUser in $AllUOSUsers) { #0
   if ($UOSUser.wWWHomePage) {#1 є табельний номер
		...
		elseif ($UserCSVString.Count -ge 2) {
           ...
           ("Помилка у файлі експорту - більше 1 запису с табельним номером",$UOSUser.wWWHomePage) |Out-File $LogsFile -Append         
           } 
			else { 
